...
Solution: Maintain a path stack `std::vector<std::pair<int,int>> path_stack` to record the path the drone has passed, and trace back one by one step to the last cross-section that has not been fully explored. This is implemented in src/block.h file.
...
```
EEP520_FinalProject
   |--src    //source file containing block.h and my_robot.h
   |   |--block.h //drone behaviour with DFS
   |   |--block.cc //nothing
   |   |--my_robot.h  //wall generate with Prim's algorithm
   |   |--my_robot.cc //nothing
   |   |--README.md
   |--defs    //physical design
   |   |--block.json //drone physical define
   |   |--my_robot.json  //wall physical define
   |   |--README.md
   |--lib     //lib
   |   |--...
